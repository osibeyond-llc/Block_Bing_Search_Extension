...
if(($ans -eq 'y') -Or ($ans -eq 'yes') -Or ($ans -eq 'ye')){
    Write-Host "`n Continuing...";
}
else { Write-Host "Do this on the other server. Exiting..."; exit; }
...
#Make the new GPO
New-GPO -Name "Computer: Block Bing Search Extension" -Server $FSMOserver.Name
